...
public class UserMealsUtil {
    public static void main(String[] args) {
        List<UserMeal> mealList = Arrays.asList(
                ...
                new UserMeal(LocalDateTime.of(2014, Month.JUNE, 1,17,0), "Завтрак2", 10000)
        );
        List<UserMealWithExceed>  filteredWithExceeded=getFilteredWithExceeded(mealList, LocalTime.of(7, 0), LocalTime.of(12,0), 2000);
        filteredWithExceeded
                .stream()
                .forEach(o->{
                    System.out.println(o.getDateTime() + " " + o.getDescription() + " " + o.getCalories() + " " + o.isExceed());
                });
//        .toLocalDate();
//        .toLocalTime();
    }
    ...
    public static List<UserMealWithExceed>  getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate,Integer> caloriesInTheDate = mealList
                .stream()
                .collect(Collectors.toMap(p->p.getDateTime().toLocalDate() ,p->p.getCalories(),
                        (cal1,cal2)->cal1 + cal2));
        return mealList
                .stream()
                .filter(o ->(o.getDateTime().toLocalTime().isAfter(startTime)&&o.getDateTime().toLocalTime().isBefore(endTime)))
                .map(o->{
